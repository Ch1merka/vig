    ...
    private void Update() {
        ...
        if (receiveInputsFromInputManager) {
            ...
            if (smoothInputs) {
                ...
                inputs.throttleInput = Mathf.MoveTowards(inputs.throttleInput, Input.GetAxis("Vertical"), Time.deltaTime * smoothingFactor);
                inputs.steerInput = Mathf.MoveTowards(inputs.steerInput, Input.GetAxis("Horizontal"), Time.deltaTime * smoothingFactor);
